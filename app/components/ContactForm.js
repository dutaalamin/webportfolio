      ...
      <form ref={form} onSubmit={sendEmail} className="space-y-4 text-xs md:text-sm">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex flex-col gap-2 flex-1">
            ...
            <input type="text" name="user_name" required className="w-full text-[8px] md:text-[10px] border-2 border-black p-2 bg-gray-50 focus:bg-white outline-none focus:border-[#f8b800]" placeholder="Name" />
          </div>
          <div className="flex flex-col gap-2 flex-1">
            <label className="text-[10px] text-gray-500 uppercase">Email</label>
            <input type="email" name="user_email" required className="w-full text-[8px] md:text-[10px] border-2 border-black p-2 bg-gray-50 focus:bg-white outline-none focus:border-[#f8b800]" placeholder="Email" />
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-[10px] text-gray-500 uppercase">Message</label>
          <textarea name="message" required rows="4" className="w-full text-[8px] md:text-[10px] border-2 border-black p-2 bg-gray-50 focus:bg-white outline-none focus:border-[#f8b800] resize-none" placeholder="What's on your mind?"></textarea>
        </div>
