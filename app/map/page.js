...
export default function WorldMapPage() {
  ...
  const [heroPos, setHeroPos] = useState({ x: 50, y: 45 });
  ...
  return (
    <main className="relative w-full h-screen overflow-hidden font-pressStart select-none">
        ...
        {paths.map((path, index) => {
          ...
          return (
            <line
              ...
              x2={to.x + 4}
              y2={to.y + 5}
              stroke={isHighlighted ? '#f8b800' : 'rgba(0,0,0,0.2)'}
        ...
        {mapLocations.map((location, index) => {
          ...
          return (
            ...
            >
              {/* Node Circle */}
              <div
                className={`relative w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full border-4 flex items-center justify-center transition-all duration-300 mx-auto ${isHovered ? 'scale-110 -translate-y-2' : ''}`}
                style={{
              ...
              <div className={`absolute left-1/2 -translate-x-1/2 mt-3 whitespace-nowrap transition-all duration-300 ${isHovered ? 'translate-y-1' : ''}`}>
                <div className="bg-white px-3 py-2 md:px-4 md:py-2 border-4 border-black text-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  <p className="text-[6px] sm:text-[7px] md:text-[9px] text-black font-bold tracking-wide">{location.name}</p>
                  <p className={`text-[5px] sm:text-[6px] md:text-[7px] text-gray-500 mt-1 transition-all duration-300 overflow-hidden ${isHovered ? 'max-h-10 opacity-100' : 'max-h-0 opacity-0'}`}>
                    {location.subtitle}
                  </p>
                </div>
              </div>
        ...
        <div className="relative">
          ...
          {!isWalking && (
            <div className="absolute -top-5 left-1/2 -translate-x-1/2 whitespace-nowrap">
              <div className="bg-white text-black text-[5px] md:text-[6px] px-2 py-0.5 border-2 border-black font-bold shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                DUTA
              </div>
            </div>
          )}
      ...
      {/* Bottom Info Bar */}
      <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-30 transition-all duration-1000 delay-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <div className="bg-white px-4 py-2 md:px-8 md:py-3 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center gap-4 md:gap-8">
          <div className="flex items-center gap-2">
            <span className="text-[8px] md:text-[10px] text-gray-400">🎮</span>
            <span className="text-[6px] md:text-[8px] text-black">CLICK A STAGE TO BEGIN</span>
          </div>
          <div className="w-px h-4 bg-gray-300" />
          <div className="flex items-center gap-2">
            <span className="text-[8px] md:text-[10px] text-gray-400">📍</span>
            <span className="text-[6px] md:text-[8px] text-black">
